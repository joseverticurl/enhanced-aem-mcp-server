...
export class AEMConnector {
            ...
            try {
                ...
                if (content) {
                    ...
                    if (elementsMode === 'sling-flat') {
                        ...
                        const payload = { 'jcr:primaryType': 'nt:unstructured' };
                    ...
                    if (elementsMode === 'sling-child' || (!cfManagerSucceeded && elementsMode !== 'sling-flat')) {
                        ...
                        for (const [elementName, elementValue] of Object.entries(content)) {
                            ...
                            if (typeof elementValue === 'string')
                                elem.append('dataType', 'string');
                            else if (typeof elementValue === 'number')
                                elem.append('dataType', 'number');
                            else if (typeof elementValue === 'boolean')
                                elem.append('dataType', 'boolean');
                            else
                                elem.append('dataType', 'string');
     ...
     */
    formatContentElementsForSling(content, model) {
        if (!content)
            return {};
        const formatted = {};
        const currentTime = new Date().toISOString();
     ...
     */
    async getContentFragment(fragmentPath) {
        return safeExecute(async () => {
            if (!isValidContentPath(fragmentPath)) {
                throw createAEMError(AEM_ERROR_CODES.INVALID_PARAMETERS, 'Invalid fragment path', { fragmentPath });
            }
            ...
            try {
                ...
                try {
                    ...
                    for (const [elementName, elementData] of Object.entries(elements)) {
                        if (elementName === 'jcr:primaryType' ||
                        ...
                        if (typeof elementData !== 'object') {
                            content[elementName] = elementData;
                            console.log(`   📝 Element found: ${elementName} = ${elementData}`);
                        }
                        else if (elementData && typeof elementData === 'object' && elementData.value !== undefined) {
                            // Fallback for nested structure
                            content[elementName] = elementData.value;
                            console.log(`   📝 Element found (nested.value): ${elementName} = ${elementData.value}`);
                        }
                    }
                }
                catch (elementsError) {
    ...
    async listContentFragments(params) {
        return safeExecute(async () => {
            ...
            const fragments = response.data.results.map((result) => ({
                path: result.path,
     ...
     */
    async getExperienceFragment(fragmentPath) {
        return safeExecute(async () => {
            if (!isValidContentPath(fragmentPath)) {
                throw createAEMError(AEM_ERROR_CODES.INVALID_PARAMETERS, 'Invalid fragment path', { fragmentPath });
            }
     ...
     */
    async getExperienceFragmentVariants(fragmentPath) {
        return safeExecute(async () => {
            if (!isValidContentPath(fragmentPath)) {
                throw createAEMError(AEM_ERROR_CODES.INVALID_PARAMETERS, 'Invalid fragment path', { fragmentPath });
            }
            ...
            const variantDetails = await Promise.all(variants.map(async (variantName) => {
                try {
