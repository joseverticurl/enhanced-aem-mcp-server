...
app.get('/api/methods', async (req, res) => {
    try {
        ...
        const categorizedMethods = methods.reduce((acc, method) => {
            const category = method.name.includes('ContentFragment') ? 'Content Fragment Operations' :
                method.name.includes('ExperienceFragment') ? 'Experience Fragment Operations' :
                    method.name.includes('Page') ? 'page' :
                        method.name.includes('Component') ? 'component' :
