...
export class AEMConnector {
  ...
  /**
   * Create a new Content Fragment with proper AEM node structure
   * Uses hybrid approach: Sling POST for structure + AEM CF Manager for content
   */
  async createContentFragment(params: {
    parentPath: string;
    title: string;
    description?: string;
    model?: string;
    ...
    locale?: string;
    elementsMode?: 'cf-manager' | 'sling-child' | 'sling-flat';
  }): Promise<object> {
    return safeExecute<object>(async () => {
      const { parentPath, title, description, model, content, tags, metadata, locale = 'en', elementsMode = 'sling-flat' } = params;
      
      if (!isValidContentPath(parentPath)) {
        throw createAEMError(AEM_ERROR_CODES.INVALID_PARAMETERS, 'Invalid parent path for content fragment', { parentPath });
      }

      const client = this.createAxiosInstance();
      const fragmentName = title.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
      const fragmentPath = `${parentPath}/${fragmentName}`;

      try {
        // First, check if parent path exists and create if needed
        console.log(`🔍 Checking if parent path exists: ${parentPath}`);
        try {
          await client.get(`${parentPath}.json`);
          console.log(`✅ Parent path exists: ${parentPath}`);
        } catch (error: any) {
          console.error(`❌ Parent path does not exist: ${parentPath}`);
          console.log(`🛠️ Attempting to create parent path structure...`);
          ...
          for (const part of pathParts) {
            currentPath += `/${part}`;
            try {
              await client.get(`${currentPath}.json`);
              console.log(`✅ Path exists: ${currentPath}`);
            } catch (pathError: any) {
              if (pathError.response?.status === 404) {
                console.log(`📁 Creating missing path: ${currentPath}`);
                const pathFormData = new URLSearchParams();
                pathFormData.append('jcr:primaryType', 'sling:Folder');
                
                await client.post(currentPath, pathFormData, {
                  headers: { 
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'Accept': 'application/json'
                  }
        ...
        // Extract project name from path for model configuration
        const projectName = parentPath.split('/')[3] || 'default';
        const currentTime = new Date().toISOString();

        console.log(`🚀 Starting Content Fragment creation at: ${fragmentPath}`);

        // Step 1: Create the main dam:Asset node
        console.log(`📝 Step 1: Creating main dam:Asset node...`);
        const mainNodeFormData = new URLSearchParams();
        mainNodeFormData.append('jcr:primaryType', 'dam:Asset');
        mainNodeFormData.append('jcr:mixinTypes', 'mix:referenceable');
        mainNodeFormData.append('jcr:mixinTypes', 'mix:versionable');
        
        await client.post(fragmentPath, mainNodeFormData, {
          headers: { 
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json'
          }
        });
        console.log(`✅ Main node created: ${fragmentPath}`);

        // Step 2: Create jcr:content node
        console.log(`📝 Step 2: Creating jcr:content node...`);
        const contentNodeFormData = new URLSearchParams();
        contentNodeFormData.append('jcr:primaryType', 'dam:AssetContent');
        contentNodeFormData.append('jcr:title', title);
        contentNodeFormData.append('dam:runDMProcess', 'true');
        contentNodeFormData.append('dam:runPostProcess', 'true');
        contentNodeFormData.append('contentFragment', 'true');
        contentNodeFormData.append('cq:name', fragmentName);
        contentNodeFormData.append('cq:parentPath', parentPath);
        
        await client.post(`${fragmentPath}/jcr:content`, contentNodeFormData, {
          headers: { 
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json'
          }
        });
        console.log(`✅ Content node created: ${fragmentPath}/jcr:content`);

        // Step 3: Create metadata node
        console.log(`📝 Step 3: Creating metadata node...`);
        const metadataFormData = new URLSearchParams();
        metadataFormData.append('jcr:primaryType', 'nt:unstructured');
        metadataFormData.append('jcr:mixinTypes', 'cq:Taggable');
        metadataFormData.append('dc:title', title);
        metadataFormData.append('dc:description', description || '');
        metadataFormData.append('dam:scene7PublishPending', 'true');
        
        if (tags && tags.length > 0) {
          tags.forEach(tag => {
            // Only set valid fully-qualified tags (e.g., namespace:path)
            if (typeof tag === 'string' && tag.includes(':')) {
              metadataFormData.append('cq:tags', tag);
            } else {
              console.warn(`Skipping invalid tag value: ${tag}`);
            }
          });
        }
        
        await client.post(`${fragmentPath}/jcr:content/metadata`, metadataFormData, {
          headers: { 
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json'
          }
        });
        console.log(`✅ Metadata node created: ${fragmentPath}/jcr:content/metadata`);

        // Step 4: Create data node
        console.log(`📝 Step 4: Creating data node...`);
        const dataFormData = new URLSearchParams();
        dataFormData.append('jcr:primaryType', 'nt:unstructured');
        dataFormData.append('_strucVersion', '1');
        dataFormData.append('cq:model', model || `/conf/${projectName}/settings/dam/cfm/models/default-model`);
        
        await client.post(`${fragmentPath}/jcr:content/data`, dataFormData, {
          headers: { 
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json'
          }
        });
        console.log(`✅ Data node created: ${fragmentPath}/jcr:content/data`);

        // Step 5: Create master node
        console.log(`📝 Step 5: Creating master node...`);
        const masterFormData = new URLSearchParams();
        masterFormData.append('jcr:primaryType', 'nt:unstructured');
        masterFormData.append('jcr:mixinTypes', 'cq:Taggable');
        masterFormData.append('jcr:mixinTypes', 'dam:cfVariationNode');
        
        await client.post(`${fragmentPath}/jcr:content/data/master`, masterFormData, {
          headers: { 
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json'
          }
        });
        console.log(`✅ Master node created: ${fragmentPath}/jcr:content/data/master`);

        // Step 6: Write elements per selected mode (default: sling-flat)
        if (content) {
          // 6A: sling-flat → set flat properties on master node
          if (elementsMode === 'sling-flat') {
            console.log('📝 Step 6: Writing flat properties on master (sling-flat import)...');
            const payload: Record<string, any> = { 'jcr:primaryType': 'nt:unstructured' };
            for (const [elementName, elementValue] of Object.entries(content)) {
              payload[elementName] = String(elementValue);
            }
            const importForm = new URLSearchParams();
            importForm.append(':operation', 'import');
            importForm.append(':contentType', 'json');
            importForm.append(':replace', 'true');
            importForm.append(':content', JSON.stringify(payload));
            await client.post(`${fragmentPath}/jcr:content/data/master`, importForm, {
              headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json' }
            });
            console.log('✅ Flat properties imported on master');
          }

          const attemptCfManager = elementsMode === 'cf-manager';
          let cfManagerSucceeded = false;
          if (attemptCfManager) {
            try {
              console.log(`📝 Step 6: Setting elements via CF Manager API...`);
              const contentStructure: Record<string, any> = {};
              for (const [elementName, elementValue] of Object.entries(content)) {
                contentStructure[elementName] = {
                  'jcr:primaryType': 'nt:unstructured',
                  'value': elementValue,
                  'valueType': this.determineValueType(elementValue),
                  'dataType': typeof elementValue === 'number' ? 'number' : typeof elementValue === 'boolean' ? 'boolean' : 'string'
                };
              }
              await client.post(`${fragmentPath}/jcr:content/data/master`, contentStructure, {
                headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }
              });
              console.log(`✅ Elements set via CF Manager API`);
              cfManagerSucceeded = true;
            } catch (cfErr: any) {
              console.warn(`⚠️ CF Manager attempt failed: ${cfErr.response?.status} ${cfErr.message}. Falling back to Sling child nodes.`);
            }
          }

          if (elementsMode === 'sling-child' || (!cfManagerSucceeded && elementsMode !== 'sling-flat')) {
            console.log(`📝 Step 6 (fallback): Creating element nodes under master...`);
            for (const [elementName, elementValue] of Object.entries(content)) {
              const elem = new URLSearchParams();
              elem.append('jcr:primaryType', 'nt:unstructured');
              elem.append('value', String(elementValue));
              elem.append('valueType', this.determineValueType(elementValue));
              if (typeof elementValue === 'string') elem.append('dataType', 'string');
              else if (typeof elementValue === 'number') elem.append('dataType', 'number');
              else if (typeof elementValue === 'boolean') elem.append('dataType', 'boolean');
              else elem.append('dataType', 'string');

              await client.post(`${fragmentPath}/jcr:content/data/master/${elementName}`, elem, {
                headers: { 'Content-Type': 'application/x-www-form-urlencoded', 'Accept': 'application/json' }
              });
        ...
        // Step 7: Create related and renditions nodes
        console.log(`📝 Step 7: Creating related and renditions nodes...`);
        const relatedFormData = new URLSearchParams();
        relatedFormData.append('jcr:primaryType', 'nt:unstructured');
        await client.post(`${fragmentPath}/jcr:content/related`, relatedFormData, {
          headers: { 
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json'
          }
        ...
        const renditionsFormData = new URLSearchParams();
        renditionsFormData.append('jcr:primaryType', 'nt:folder');
        await client.post(`${fragmentPath}/jcr:content/renditions`, renditionsFormData, {
          headers: { 
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json'
          }
        });
        console.log(`✅ Related and renditions nodes created`);

        // Step 8: Create indexed data structure
        console.log(`📝 Step 8: Creating indexed data structure...`);
        const indexedDataFormData = new URLSearchParams();
        indexedDataFormData.append('jcr:primaryType', 'nt:unstructured');
        await client.post(`${fragmentPath}/jcr:content/indexedData`, indexedDataFormData, {
          headers: { 
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json'
          }
        ...
        const indexedMasterFormData = new URLSearchParams();
        indexedMasterFormData.append('jcr:primaryType', 'nt:unstructured');
        indexedMasterFormData.append('jcr:mixinTypes', 'dam:IndexedFragmentData');
        indexedMasterFormData.append('@string@model', model || `/conf/${projectName}/settings/dam/cfm/models/default-model`);
        indexedMasterFormData.append('@stringArray@variations', '[]');
        
        await client.post(`${fragmentPath}/jcr:content/indexedData/master`, indexedMasterFormData, {
          headers: { 
            'Content-Type': 'application/x-www-form-urlencoded',
            'Accept': 'application/json'
          }
        });
        console.log(`✅ Indexed data structure created`);

        // Note: We intentionally do not create per-element nodes under indexedData/master

        console.log(`🎉 Content Fragment creation completed successfully!`);
        ...
        return createSuccessResponse({
          ...
          locale,
          created: true,
          method: 'sling-post-structured',
          structure: {
            mainNode: fragmentPath,
            contentNode: `${fragmentPath}/jcr:content`,
            dataNode: `${fragmentPath}/jcr:content/data`,
            masterNode: `${fragmentPath}/jcr:content/data/master`,
            elementsNode: `${fragmentPath}/jcr:content/data/master`
          }
        }, 'createContentFragment');

      } catch (error: any) {
        console.error(`❌ Content Fragment creation failed:`);
        console.error(`   Error: ${error.response?.status} - ${error.response?.data || error.message}`);
        console.error(`   Fragment path: ${fragmentPath}`);
        console.error(`   Parent path: ${parentPath}`);
        
        // Cleanup on failure
        try {
          console.log(`🧹 Attempting cleanup of failed fragment...`);
          await client.delete(fragmentPath);
          console.log(`✅ Cleanup successful`);
        } catch (cleanupError: any) {
          console.warn(`⚠️ Failed to cleanup fragment on creation error: ${cleanupError.message}`);
        }
        throw error;
      }
    }, 'createContentFragment');
  ...
  /**
   * Format content elements for Sling POST servlet
   * This method creates the proper structure that matches AEM's exact format
   */
  private formatContentElementsForSling(content?: Record<string, any>, model?: string): Record<string, any> {
    if (!content) return {};
    ...
    for (const [elementName, elementValue] of Object.entries(content)) {
      // Create the element structure that matches AEM's exact format
      // Format: elementName@LastModified, elementName@ContentType, and direct elementName
      formatted[`${elementName}@LastModified`] = currentTime;
      formatted[`${elementName}@ContentType`] = 'text/plain';
      formatted[elementName] = elementValue;
    }
  ...
  /**
   * Determine the value type for content fragment elements
   */
  private determineValueType(value: any): string {
    if (typeof value === 'string') {
      ...
      return 'string'; // Arrays are typically stored as JSON strings
    } else if (typeof value === 'object' && value !== null) {
      return 'string'; // Objects are stored as JSON strings
    }
    return 'string';
  }

  /**
   * Determine the data type based on model and value
   */
  private determineDataType(value: any, model?: string): string | undefined {
    // This could be enhanced to read from the actual content fragment model
    // For now, we'll return undefined to let AEM handle it
    return undefined;
  }

  /**
   * Update an existing Content Fragment
   */
  async updateContentFragment(params: {
    fragmentPath: string;
    title?: string;
    description?: string;
    content?: Record<string, any>;
    ...
    return safeExecute<object>(async () => {
      const { fragmentPath, title, description, content, tags, metadata } = params;
      
      if (!isValidContentPath(fragmentPath)) {
        throw createAEMError(AEM_ERROR_CODES.INVALID_PARAMETERS, 'Invalid fragment path', { fragmentPath });
      }

      const client = this.createAxiosInstance();
      
      // Get current fragment data
      const currentResponse = await client.get(`${fragmentPath}.json`);
      if (!currentResponse.data) {
        throw createAEMError(AEM_ERROR_CODES.RESOURCE_NOT_FOUND, 'Content fragment not found', { fragmentPath });
      }

      const currentData = currentResponse.data['jcr:content'] || {};
      ...
      if (title !== undefined) {
        updateData['jcr:content/metadata/dc:title'] = title;
      }
      
      if (description !== undefined) {
        updateData['jcr:content/metadata/dc:description'] = description;
      }
      
      if (tags !== undefined) {
        updateData['jcr:content/metadata/cq:tags'] = tags;
      }
      ...
      // Update the fragment
      await client.post(`${fragmentPath}.json`, updateData, {
        headers: { 'Content-Type': 'application/json' }
      });

      return createSuccessResponse({
        fragmentPath,
        title: title || currentMetadata['dc:title'],
        description: description || currentMetadata['dc:description'],
        content,
        tags: tags || currentMetadata['cq:tags'],
   ...
   */
  async deleteContentFragment(params: {
    fragmentPath: string;
    force?: boolean;
  }): Promise<object> {
    return safeExecute<object>(async () => {
      const { fragmentPath, force = false } = params;
      ...
      try {
        await client.get(`${fragmentPath}.json`);
      } catch (error: any) {
        if (error.response?.status === 404) {
          throw createAEMError(AEM_ERROR_CODES.RESOURCE_NOT_FOUND, 'Content fragment not found', { fragmentPath });
        }
  ...
  /**
   * Get Content Fragment details with proper AEM node structure
   */
  async getContentFragment(fragmentPath: string): Promise<object> {
    return safeExecute<object>(async () => {
      if (!isValidContentPath(fragmentPath)) {
        throw createAEMError(AEM_ERROR_CODES.INVALID_PARAMETERS, 'Invalid fragment path', { fragmentPath });
      }
      ...
      try {
        console.log(`🔍 Retrieving Content Fragment: ${fragmentPath}`);
        
        // Get main fragment data
        const fragmentResponse = await client.get(`${fragmentPath}.json`);
        if (!fragmentResponse.data) {
          throw createAEMError(AEM_ERROR_CODES.RESOURCE_NOT_FOUND, 'Content fragment not found', { fragmentPath });
        }
        console.log(`✅ Main fragment data retrieved`);

        // Get metadata
        let metadata: Record<string, any> = {};
        try {
          const metadataResponse = await client.get(`${fragmentPath}/jcr:content/metadata.json`);
          metadata = metadataResponse.data || {};
          console.log(`✅ Metadata retrieved`);
        } catch (metadataError: any) {
          console.warn(`⚠️ Metadata not found: ${metadataError.message}`);
        }

        // Get content elements from the master node (flat properties style)
        let content: Record<string, any> = {};
        try {
          const elementsResponse = await client.get(`${fragmentPath}/jcr:content/data/master.json`);
          const elements = elementsResponse.data || {};
          console.log(`✅ Content elements retrieved from: ${fragmentPath}/jcr:content/data/master`);

          // Extract content values from flat properties; ignore jcr/cq/dam keys and @-suffixed meta keys
          for (const [elementName, elementData] of Object.entries(elements)) {
            if (
              elementName === 'jcr:primaryType' ||
              elementName === 'jcr:mixinTypes' ||
              elementName.startsWith('jcr:') ||
              elementName.startsWith('cq:') ||
              elementName.startsWith('dam:') ||
            ...
            if (typeof elementData !== 'object') {
              content[elementName] = elementData as any;
              console.log(`   📝 Element found: ${elementName} = ${elementData}`);
            } else if (elementData && typeof elementData === 'object' && (elementData as any).value !== undefined) {
              // Fallback for nested structure
              content[elementName] = (elementData as any).value;
              console.log(`   📝 Element found (nested.value): ${elementName} = ${(elementData as any).value}`);
            }
          }
        } catch (elementsError: any) {
          console.warn(`⚠️ Content elements not found: ${elementsError.message}`);
          // Try alternative path structure for legacy fragments
          try {
            const altElementsResponse = await client.get(`${fragmentPath}/jcr:content/data/master/jcr:content/elements.json`);
            const altElements = altElementsResponse.data || {};
            console.log(`✅ Content elements retrieved from alternative path`);
            
            for (const [elementName, elementData] of Object.entries(altElements)) {
              if (elementName !== 'jcr:primaryType' && elementData && typeof elementData === 'object') {
                const element = elementData as any;
                if (element.value !== undefined) {
                  content[elementName] = element.value;
                }
              }
            }
          } catch (altError: any) {
            console.warn(`⚠️ Alternative path also failed: ${altError.message}`);
          }
        }
        ...
        let model = null;
        try {
          const modelResponse = await client.get(`${fragmentPath}/jcr:content/data.json`);
          if (modelResponse.data && modelResponse.data['cq:model']) {
            model = modelResponse.data['cq:model'];
            console.log(`✅ Model reference found: ${model}`);
          }
        } catch (modelError: any) {
          console.warn(`⚠️ Model not found: ${modelError.message}`);
        }

        // Get indexed data for additional content
        let indexedContent: Record<string, any> = {};
        try {
          const indexedResponse = await client.get(`${fragmentPath}/jcr:content/indexedData/master.json`);
          const indexedData = indexedResponse.data || {};
          console.log(`✅ Indexed data retrieved`);

          for (const [elementName, elementData] of Object.entries(indexedData)) {
            if (elementName !== 'jcr:primaryType' && elementName !== 'jcr:mixinTypes' && elementData && typeof elementData === 'object') {
              const element = elementData as any;
              // Check for indexed content values
              if (element[`@string@${elementName}`] !== undefined) {
                indexedContent[elementName] = element[`@string@${elementName}`];
                console.log(`   📝 Indexed element found: ${elementName} = ${element[`@string@${elementName}`]}`);
              }
            }
          }
        } catch (indexedError: any) {
          console.warn(`⚠️ Indexed data not found: ${indexedError.message}`);
        }

        // Merge content from both sources, prioritizing master content
        const finalContent = { ...indexedContent, ...content };

        console.log(`🎉 Content Fragment retrieval completed successfully!`);
        console.log(`   Content elements found: ${Object.keys(finalContent).length}`);
        console.log(`   Model: ${model || 'Not specified'}`);

        return createSuccessResponse({
          fragmentPath,
          title: metadata['dc:title'] || fragmentResponse.data['jcr:content']?.['jcr:title'] || 'Untitled Fragment',
          description: metadata['dc:description'] || '',
          model,
          content: finalContent,
          tags: metadata['cq:tags'] || [],
          locale: metadata['jcr:language'] || 'en',
          metadata,
          created: fragmentResponse.data['jcr:created'],
          modified: fragmentResponse.data['jcr:lastModified'],
          createdBy: fragmentResponse.data['jcr:createdBy'],
          modifiedBy: fragmentResponse.data['jcr:lastModifiedBy'],
          structure: {
            mainNode: fragmentPath,
            contentNode: `${fragmentPath}/jcr:content`,
            dataNode: `${fragmentPath}/jcr:content/data`,
            masterNode: `${fragmentPath}/jcr:content/data/master`,
            elementsNode: `${fragmentPath}/jcr:content/data/master`,
            indexedNode: `${fragmentPath}/jcr:content/indexedData/master`
          }
        }, 'getContentFragment');

      } catch (error: any) {
        console.error(`❌ Content Fragment retrieval failed:`);
        console.error(`   Error: ${error.response?.status} - ${error.response?.data || error.message}`);
        console.error(`   Fragment path: ${fragmentPath}`);
  ...
  /**
   * List Content Fragments in a directory
   */
  async listContentFragments(params: {
    ...
    return safeExecute<object>(async () => {
      const { parentPath, depth = 2, limit = 50, filter } = params;
      
      if (!isValidContentPath(parentPath)) {
        throw createAEMError(AEM_ERROR_CODES.INVALID_PARAMETERS, 'Invalid parent path', { parentPath });
      }

      const client = this.createAxiosInstance();
      
      // Use QueryBuilder to find content fragments
      const queryParams = {
        type: 'dam:Asset',
        path: parentPath,
        depth: depth,
      ...
      const response = await client.get('/bin/querybuilder.json', { params: queryParams });
      ...
      const fragments = response.data.results.map((result: any) => ({
        path: result.path,
        title: result['dc:title'] || 'Untitled Fragment',
        description: result['dc:description'] || '',
        model: result.model || null,
        tags: result['cq:tags'] || [],
        created: result['jcr:created'],
        modified: result['jcr:lastModified']
      }));
   ...
   */
  async createExperienceFragment(params: {
    parentPath: string;
    title: string;
    description?: string;
    template?: string;
    ...
    return safeExecute<object>(async () => {
      const { parentPath, title, description, template, content, tags, variants, metadata } = params;
      
      if (!isValidContentPath(parentPath)) {
        throw createAEMError(AEM_ERROR_CODES.INVALID_PARAMETERS, 'Invalid parent path for experience fragment', { parentPath });
      }
      ...
      // Create the experience fragment structure
      const fragmentData = {
        'jcr:primaryType': 'cq:Page',
        'jcr:content': {
          'jcr:primaryType': 'nt:unstructured',
          ...
          'cq:tags': tags || [],
          'cq:template': template || '/libs/cq/experience-fragments/templates/experience-fragment',
      ...
      const response = await client.post(`${fragmentPath}.json`, fragmentData, {
        headers: { 'Content-Type': 'application/json' }
      });

      return createSuccessResponse({
        fragmentPath: response.data.path || fragmentPath,
        title,
        description,
        template,
        content,
  ...
  /**
   * Update an existing Experience Fragment
   */
  async updateExperienceFragment(params: {
    fragmentPath: string;
    title?: string;
    description?: string;
    content?: Record<string, any>;
    ...
    return safeExecute<object>(async () => {
      const { fragmentPath, title, description, content, tags, variants, metadata } = params;
      ...
      const currentResponse = await client.get(`${fragmentPath}.json`);
      if (!currentResponse.data) {
        throw createAEMError(AEM_ERROR_CODES.RESOURCE_NOT_FOUND, 'Experience fragment not found', { fragmentPath });
      }
      ...
      if (title !== undefined) {
        updateData['jcr:content/jcr:title'] = title;
      }
      
      if (description !== undefined) {
        updateData['jcr:content/jcr:description'] = description;
      }
      
      if (tags !== undefined) {
        updateData['jcr:content/cq:tags'] = tags;
      }
      
      if (variants !== undefined) {
        updateData['jcr:content/xfVariants'] = variants;
      }
      
      if (content) {
        Object.entries(content).forEach(([key, value]) => {
          updateData[`jcr:content/${key}`] = value;
        });
      }
      ...
      return createSuccessResponse({
        fragmentPath,
        title: title || currentData['jcr:title'],
        description: description || currentData['jcr:description'],
        content,
        tags: tags || currentData['cq:tags'],
        variants: variants || currentData['xfVariants'],
   ...
   */
  async deleteExperienceFragment(params: {
    fragmentPath: string;
    force?: boolean;
  }): Promise<object> {
    return safeExecute<object>(async () => {
  ...
  /**
   * Get Experience Fragment details
   */
  async getExperienceFragment(fragmentPath: string): Promise<object> {
    return safeExecute<object>(async () => {
      if (!isValidContentPath(fragmentPath)) {
        throw createAEMError(AEM_ERROR_CODES.INVALID_PARAMETERS, 'Invalid fragment path', { fragmentPath });
      }
      ...
      const client = this.createAxiosInstance();
      const response = await client.get(`${fragmentPath}.json`);
      
      if (!response.data) {
        throw createAEMError(AEM_ERROR_CODES.RESOURCE_NOT_FOUND, 'Experience fragment not found', { fragmentPath });
      }

      const fragmentData = response.data;
      const content = fragmentData['jcr:content'] || {};

      return createSuccessResponse({
        fragmentPath,
        title: content['jcr:title'] || 'Untitled Fragment',
        description: content['jcr:description'] || '',
        template: content['cq:template'] || '',
        resourceType: content['sling:resourceType'] || '',
        tags: content['cq:tags'] || [],
        variants: content['xfVariants'] || [],
        content: content,
        created: fragmentData['jcr:created'],
        modified: fragmentData['jcr:lastModified'],
        createdBy: fragmentData['jcr:createdBy'],
        modifiedBy: fragmentData['jcr:lastModifiedBy']
      }, 'getExperienceFragment');
    }, 'getExperienceFragment');
  }

  /**
   * List Experience Fragments in a directory
   */
  async listExperienceFragments(params: {
    ...
    return safeExecute<object>(async () => {
      ...
      // Use QueryBuilder to find experience fragments
      const queryParams = {
        type: 'cq:Page',
        path: parentPath,
        depth: depth,
        ...
        'property.value': 'cq/experience-fragments/components/xfpage'
      };
      ...
      const fragments = response.data.results.map((result: any) => ({
        path: result.path,
        title: result['jcr:title'] || 'Untitled Fragment',
        description: result['jcr:description'] || '',
        template: result['cq:template'] || '',
        resourceType: result['sling:resourceType'] || '',
        tags: result['cq:tags'] || [],
        variants: result['xfVariants'] || [],
        created: result['jcr:created'],
        modified: result['jcr:lastModified']
      }));
  ...
  /**
   * Create a variant of an Experience Fragment
   */
  async createExperienceFragmentVariant(params: {
    fragmentPath: string;
    variantName: string;
    variantType: string;
    content?: Record<string, any>;
    ...
    return safeExecute<object>(async () => {
      const { fragmentPath, variantName, variantType, content, metadata } = params;
      ...
      const client = this.createAxiosInstance();
      const variantPath = `${fragmentPath}/${variantName}`;
      ...
      const response = await client.post(`${variantPath}.json`, variantData, {
        headers: { 'Content-Type': 'application/json' }
      });

      return createSuccessResponse({
        variantPath: response.data.path || variantPath,
        variantName,
        variantType,
        content,
        metadata,
        created: true
      }, 'createExperienceFragmentVariant');
    }, 'createExperienceFragmentVariant');
  }

  /**
   * Get all variants of an Experience Fragment
   */
  async getExperienceFragmentVariants(fragmentPath: string): Promise<object> {
    return safeExecute<object>(async () => {
      if (!isValidContentPath(fragmentPath)) {
        throw createAEMError(AEM_ERROR_CODES.INVALID_PARAMETERS, 'Invalid fragment path', { fragmentPath });
      }
      ...
      const fragmentData = response.data;
      const content = fragmentData['jcr:content'] || {};
      const variants = content['xfVariants'] || [];

      // Get details for each variant
      const variantDetails = await Promise.all(
        variants.map(async (variantName: string) => {
          try {
            const variantResponse = await client.get(`${fragmentPath}/${variantName}.json`);
            const variantData = variantResponse.data;
            const variantContent = variantData['jcr:content'] || {};
            
            return {
              name: variantName,
              path: `${fragmentPath}/${variantName}`,
              title: variantContent['jcr:title'] || variantName,
              variantType: variantContent['variantType'] || 'default',
              content: variantContent,
              created: variantData['jcr:created'],
              modified: variantData['jcr:lastModified']
            };
          } catch (error) {
            return {
              name: variantName,
              path: `${fragmentPath}/${variantName}`,
              error: 'Failed to load variant details'
            };
          }
        })
      );
      ...
      return createSuccessResponse({
        ...
        total: variantDetails.length
      }, 'getExperienceFragmentVariants');
    }, 'getExperienceFragmentVariants');
  }
} 