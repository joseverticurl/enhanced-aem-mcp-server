...
#### Content Fragment Operations (5 methods)
- **Content Fragment CRUD**: Create, update, delete, and retrieve content fragments
- **Content Management**: Manage structured content with models and metadata
- **Content Discovery**: List and search content fragments with filtering
- **Model Integration**: Support for content fragment models and validation

#### Experience Fragment Operations (7 methods)
- **Experience Fragment CRUD**: Create, update, delete, and retrieve experience fragments
- **Variant Management**: Create and manage multiple variants of experience fragments
- **Template Integration**: Support for custom templates and resource types
- **Content Management**: Manage fragment content, tags, and metadata
- **Fragment Discovery**: List and search experience fragments with filtering
...
curl -u admin:admin \
  ...
  -d '{
    ...
    "params": {
      "parentPath": "/content/dam/mysite/content-fragments",
      "title": "Product Description",
      "description": "Detailed product information for e-commerce",
      "model": "/conf/mysite/settings/dam/models/product-model",
      "content": {
      ...
      "tags": ["product", "e-commerce", "widget"],
      "metadata": {
...
#### 7. Create an Experience Fragment
```bash
curl -u admin:admin \
  -X POST http://localhost:3001/mcp \
  -H 'Content-Type: application/json' \
  -d '{
    "jsonrpc": "2.0",
    "id": 7,
    "method": "createExperienceFragment",
    "params": {
      "parentPath": "/content/experience-fragments/mysite",
      "title": "Hero Banner",
      "description": "Main hero banner for homepage",
      "template": "/libs/cq/experience-fragments/templates/experience-fragment",
      "content": {
      ...
      "variants": ["web", "mobile", "tablet"]
    }
  }'
```

#### 8. Create Experience Fragment Variant
```bash
curl -u admin:admin \
  -X POST http://localhost:3001/mcp \
  -H 'Content-Type: application/json' \
  -d '{
    "jsonrpc": "2.0",
    "id": 8,
    "method": "createExperienceFragmentVariant",
    "params": {
      "fragmentPath": "/content/experience-fragments/mysite/hero-banner",
      "variantName": "mobile",
      "variantType": "mobile",
      "content": {
        "heroTitle": "Mobile Hero",
        "heroSubtitle": "Optimized for mobile",
        "ctaText": "Learn More",
        "ctaLink": "/mobile-products"
      },
      "metadata": {
        "deviceType": "mobile",
        "breakpoint": "768px"
...
- `createContentFragment` - Create new content fragments with models and content
- `updateContentFragment` - Update existing content fragment properties and content
- `deleteContentFragment` - Remove content fragments from DAM
- `getContentFragment` - Retrieve content fragment details and content
- `listContentFragments` - List and search content fragments with filtering

#### Experience Fragment Operations
- `createExperienceFragment` - Create new experience fragments with templates
- `updateExperienceFragment` - Update existing experience fragment properties
- `deleteExperienceFragment` - Remove experience fragments
- `getExperienceFragment` - Retrieve experience fragment details and content
- `listExperienceFragments` - List and search experience fragments with filtering
- `createExperienceFragmentVariant` - Create variants of experience fragments
- `getExperienceFragmentVariants` - Get all variants of an experience fragment
...
```javascript
// 1. Create content fragments for product catalog
const productFragment = await createContentFragment({
  parentPath: '/content/dam/mysite/content-fragments/products',
  ...
  tags: ['premium', 'widget', 'electronics'],
  metadata: {
    category: 'electronics',
    brand: 'WidgetCorp',
...
await updateContentFragment({
  fragmentPath: productFragment.data.fragmentPath,
  content: {
    price: '349.99', // Updated price
    features: ['Premium Feature 1', 'Premium Feature 2', 'New Feature 3']
  },
  tags: ['premium', 'widget', 'electronics', 'featured']
});

// 3. List all product fragments
const productFragments = await listContentFragments({
  parentPath: '/content/dam/mysite/content-fragments/products',
  depth: 2,
  limit: 50,
  filter: 'premium'
...
```javascript
// 1. Create experience fragment for marketing campaign
const campaignFragment = await createExperienceFragment({
  parentPath: '/content/experience-fragments/mysite/campaigns',
  title: 'Summer Sale Campaign',
  description: 'Summer sale promotion for all products',
  template: '/libs/cq/experience-fragments/templates/experience-fragment',
  content: {
    campaignTitle: 'Summer Sale 2024',
    campaignSubtitle: 'Up to 50% off on selected items',
    ctaText: 'Shop Now',
    ctaLink: '/summer-sale',
    bannerColor: '#FF6B35'
  },
  tags: ['campaign', 'summer', 'sale', 'marketing'],
  variants: ['web', 'mobile', 'email']
});

// 2. Create variants for different channels
await createExperienceFragmentVariant({
  fragmentPath: campaignFragment.data.fragmentPath,
  variantName: 'mobile',
  variantType: 'mobile',
  content: {
    campaignTitle: 'Summer Sale',
    campaignSubtitle: '50% Off!',
    ctaText: 'Shop',
    ctaLink: '/mobile-summer-sale',
    bannerColor: '#FF6B35'
  },
  metadata: {
    deviceType: 'mobile',
...
// 3. Update main fragment with new content
await updateExperienceFragment({
  fragmentPath: campaignFragment.data.fragmentPath,
  content: {
    campaignTitle: 'Extended Summer Sale 2024',
    campaignSubtitle: 'Up to 60% off - Extended until August!',
    endDate: '2024-08-31'
  },
  tags: ['campaign', 'summer', 'sale', 'marketing', 'extended']
});

// 4. List all campaign fragments
const campaignFragments = await listExperienceFragments({
  parentPath: '/content/experience-fragments/mysite/campaigns',
  depth: 2,
  limit: 20,
  filter: 'summer'
